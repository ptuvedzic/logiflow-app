...
create policy notifications_operations_select_own
...
create function public.acknowledge_notification(notification_id uuid)
returns timestamptz
language plpgsql
volatile
security definer
...
begin
  ...
  if actor_is_active is distinct from true
    or actor_role not in ('admin', 'dispatcher') then
    return null;
  end if;
